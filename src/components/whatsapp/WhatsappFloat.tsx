...
export default function WhatsappFloat() {
  ...
  if (!phoneNumber) {
    ...
    // Afficher un message visible en dev pour débugger
    if (process.env.NODE_ENV === 'development') {
      return (
        <div className="fixed bottom-6 right-6 z-50 bg-red-600 text-white p-4 rounded-lg text-xs">
          ⚠️ Variable NEXT_PUBLIC_WHATSAPP_NUMBER manquante
        </div>
      );
    }
    return null;
  ...
  return (
    <a
      ...
      className="fixed bottom-6 right-6 z-50 flex items-center justify-center w-14 h-14 bg-green-600 hover:bg-green-500 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 group"
